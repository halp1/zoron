...
const validateNotificationOptions = (options: any): true | string => {
  ...
  if (actions !== undefined) {
    ...
    for (const action of actions) {
      ...
      if (typeof action.title !== "string")
        return 'Action must have a string property "title".';
  ...
  if (tag !== undefined && typeof tag !== "string")
    return "Tag must be a string if provided.";
...
export const POST: RequestHandler = async ({ request, params: { id } }) => {
  ...
  const validate = push.options
    ? validateNotificationOptions(push.options)
    : true;
