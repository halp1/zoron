...
export const generateSchedule = async (session: Session) => {
  ...
  try {
    ...
    return await aspen.schedule.pdf(session.user.session.cookie);
  } catch {
    ...
    return await aspen.schedule.pdf(aspenSession.cookie);
  }
};
