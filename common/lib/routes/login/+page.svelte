...
<main class="flex h-screen w-screen flex-col items-center justify-center px-5">
  ...
  <form onsubmit={handleSubmission} class="flex w-96 flex-col gap-2">
    ...
    <input
      class="mb-3 w-full rounded-lg border-2 border-dashed {$theme === 'amoled'
        ? 'border-white'
        : 'border-blue-400'} bg-transparent px-5 py-3 outline-none focus-within:border-solid focus-within:outline-none"
      name="password"
      bind:value={password}
    ...
    <div class="flex items-center">
      <a
        ...
        class="{$theme === 'amoled'
          ? 'text-white'
          : 'text-slate-400'} underline">Forgot password?</a
      >
      <a
        href="/register"
        class="ml-auto {$theme === 'amoled'
          ? 'text-white'
          : 'text-slate-400'} underline">Register</a
      >
    </div>
  </form>
</main>
